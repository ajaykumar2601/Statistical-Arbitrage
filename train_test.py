...
def train(model,
          ...
          , optimizer_name = "Adam", optimizer_opts = {"lr": 0.0001},
          ...
          force_retrain = True,
          objective = "sharpe", objective_secondary = 'information_ratio',vari=None): 
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(int((T-lookback)/batchsize)+1):
            ...
            #weights= torch.zeros((min(batchsize*(i+1),T-lookback)-batchsize*i, N), device=device)
            weights= 2*torch.rand((min(batchsize*(i+1),T-lookback)-batchsize*i, N), device=device)-1
            weights = weights/(weights.sum()+0.00000000001)
            if epoch == 0 and i == 0:
            ...
            #all sticks selected below therefore no filtering here.
            ...
            try:
                excess_rets_train = torch.sum(weights*torch.tensor(excess_returns_data[lookback+batchsize*i:min(lookback+batchsize*(i+1),T),:],device=device),axis=1) 
            except:
                print("Here")
            # # no minibatch
            # if quantile is None:
            ...
            mean_excess_ret = torch.mean(excess_rets_train)
            std_excess = torch.std(excess_rets_train)
            
            
            if objective == "sharpe" and objective_secondary == 'information_ratio':
                #loss = -mean_ret/std
                print("ALERT!!!!")
            
            elif objective == "returns_max":
                loss = -mean_excess_ret
            
            
            
            elif objective == "sharpe" and objective_secondary != 'information_ratio':
        ...
        if log_dev_progress and epoch % log_dev_progress_freq == 0:
            ...
            if data_dev is not None:
                rets_dev,dev_loss,dev_sharpe,dev_turnovers,dev_short_proportions,weights_dev,a2t,exc_ret = \
                    get_returns(model,
            ...
            if objective_secondary=="information_ratio":
            ...
            if early_stopping:
                if dev_loss < min_dev_loss:
                    ...
                    torch.save(checkpoint, os.path.join(output_path, f'{vari}Checkpoint-{model.random_seed}_seed_{model_tag}.tar'))
                else:
                    patience += 1
...
def test(Data, 
    ...
    excess_returns_data = excess_returns_data[:,assets_to_trade]# newly created
    T,N = Data.shape
    excess_returns = np.zeros(T-length_training)# newly created
    ...
    # run train/test over dataset 0-1000***125-1125
    for t in range(int( (T-length_training) / retrain_freq ) + 1): 
        
        print (f"running test in {t}/{int( (T-length_training) / retrain_freq ) + 1} this many times test will play")
        ...
        data_train_t = Data[t*retrain_freq:length_training+t*retrain_freq] # 0-1000, 250-1250, 500-1500
        excess_returns_data_train_t = excess_returns_data[t*retrain_freq:length_training+t*retrain_freq] 
        data_test_t = Data[length_training+t*retrain_freq-lookback:min(length_training+(t+1)*retrain_freq,T)] # 970-1250; 1220-1500
        excess_returns_data_test_t = excess_returns_data[length_training+t*retrain_freq-lookback:min(length_training+(t+1)*retrain_freq,T)] # 970-1250; 1220-1500# newly created
        #first trading signal is generated on 30th day. therefore testdata is from 970 and not 1000. a good test is to see the (n-1)
        # bound of test to be nth bound of train.
        ...
        if rolling_retrain or t == 0:
            #here we expect to train the model 
            
            model_t = model

            ##Check this condition above, it should be above model?
            print (f"{t}/21 the value of t started")
            ...
            rets_t,turns_t,shorts_t,weights_t,a2t, model_t = train(model_t,
                                              ...
                                              excess_returns_data = excess_returns_data_train_t,# newly created
    ...
    ax = plt.gca()
    ax.xaxis.set_major_locator(DayLocator(interval=180))  # Show ticks every 30 days
    ax.xaxis.set_major_formatter(DateFormatter('%Y-%m-%d'))  # Format tick labels as YYYY-MM-DD
    ...
    plt.savefig(os.path.join(output_path, model_tag + f"{vari}_cumulative-returns.png"), bbox_inches='tight')
    #plt.show()
    ...
    plt.tight_layout()  # Improve spacing and prevent overlap
    plt.savefig(os.path.join(output_path, model_tag + f"{vari}_turnover.png"))
    #plt.show()
    ...
    plt.tight_layout()  # Improve spacing and prevent overlap
    plt.savefig(os.path.join(output_path, model_tag + f"{vari}_short-proportion.png"))
    #plt.show()
    ...
    plt.savefig(os.path.join(output_path, model_tag + f"{vari}_excess_cumulative-returns.png"), bbox_inches='tight')
    #plt.show()
    
    np.save(os.path.join(output_path, f'{vari}WeightsComplete_' + model_tag + '.npy'), all_weights)
    np.save(os.path.join(output_path, f'{vari}cumalphaRets_' + model_tag + '.npy'), cumRets)
    np.save(os.path.join(output_path, f'{vari}alphaReturns' + model_tag + '.npy'), returns)
    np.save(os.path.join(output_path, f'{vari}short_' + model_tag + '.npy'), short_proportions)
    np.save(os.path.join(output_path, f'{vari}turnover' + model_tag + '.npy'), turnovers)

    np.save(os.path.join(output_path, f'{vari}ExcessReturns' + model_tag + '.npy'), excess_returns)
    np.save(os.path.join(output_path, f'{vari}CumulativeExcessReturns' + model_tag + '.npy'), cumExRets)
...
def estimate(Data, 
    ...
    data_train = Data[estimate_start_idx:min(estimate_start_idx+length_training,T)]#0-1000
    data_dev = Data[estimate_start_idx+length_training-lookback:min(estimate_start_idx+length_training+test_size,T)]#970-1125
